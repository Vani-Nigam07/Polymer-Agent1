...
from OpenMacromolecularGenome.generation.selfies.constants import (
    ELEMENTS,
    INDEX_ALPHABET,
    INDEX_CODE,
    ORGANIC_SUBSET,
...
from OpenMacromolecularGenome.generation.selfies.mol_graph import Atom
from OpenMacromolecularGenome.generation.selfies.utils.smiles_utils import smiles_to_bond
...
def _process_atom_selfies_no_cache(symbol):
    if symbol == '[*]':
        m = SELFIES_ATOM_PATTERN.match('[F]')
    elif symbol == '[/*]':
        m = SELFIES_ATOM_PATTERN.match('[/F]')
    elif symbol == '[\*]':
        m = SELFIES_ATOM_PATTERN.match('[\F]')
    elif symbol == '[=*]':
        m = SELFIES_ATOM_PATTERN.match('[=O]')
    elif symbol == '[#*]':
        m = SELFIES_ATOM_PATTERN.match('[#N]')
    else:
    ...
    elif symbol[1 + len(bond_char):-1] in ASTERISK:
        atom_fac = functools.partial(Atom, element=element, is_aromatic=False)
        return smiles_to_bond(bond_char), atom_fac
